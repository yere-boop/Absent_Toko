...
async function run() {
  ...
  for (const emp of sampleEmployees) {
    const existing = await Employee.findByCode(emp.employee_code);
    if (!existing) {
      const result = await Employee.create(emp);
  ...
  const statuses = ['Hadir', 'Hadir', 'Hadir', 'Hadir', 'Tidak Hadir', 'Setengah Hari', 'Lembur'];
  ...
  for (let dayOffset = 0; dayOffset < 5; dayOffset++) {
    ...
    for (let index = 0; index < employeeIds.length; index++) {
      ...
      try {
        const exists = await Attendance.existsForDate(empId, dateStr);
        if (!exists) {
